...
const Sidebar: React.FC<SidebarProps> = ({ activeItem = 'home', onItemClick }) => {
  ...
  return (
    <aside className="w-72 h-screen bg-secondary-light shadow-xl border-r border-white/10 flex flex-col overflow-hidden">
      {/* Logo/Brand Section */}
      <div className="flex-shrink-0 p-8 border-b border-white/10">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-2xl font-bold text-white tracking-tight">AyanBridge</span>
        </div>
      </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center px-4 py-3 text-sm font-medium transition-all duration-200 group rounded-xl relative ${
                  activeItem === item.id
